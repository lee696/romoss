...
class AddrController extends BaseContoller {
    ...
    public function defaultadd(){
      ...
      $user_id = getUserId();
      $addid=I('address_id');
      M('Address')->where(['user_id'=>$user_id,'address_id'=>$addid])->save(['is_default'=>1]);
      M('Address')->where(['user_id'=>$user_id,'address_id'=>array('neq',$addid)])->save(['is_default'=>0]);
      sendResult();
    }
